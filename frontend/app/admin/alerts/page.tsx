                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      ...
                      {alert.is_acknowledged && (
                        <span className="px-2 py-1 bg-secondary text-secondary-foreground rounded-full text-xs font-medium flex items-center gap-1 whitespace-nowrap">
                          <CheckCircle className="h-3 w-3" />
                          Acknowledged
                        </span>
                      )}
                    </div>
                    <CardTitle className="text-xl mb-2 break-words">{alert.title}</CardTitle>
                    <p className="text-muted-foreground whitespace-pre-wrap break-words">{alert.message}</p>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pt-4 border-t gap-2">
                  <div className="flex flex-col gap-2 text-sm text-muted-foreground min-w-0">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="break-words">{formatDate(alert.created_at)}</span>
                    </div>
                    {alert.expires_at && (
                      <div className="flex items-center gap-1">
                        <span className="break-words">Expires: {formatDate(alert.expires_at)}</span>
                      </div>
                    )}
                  </div>
                  {alert.is_active && !alert.is_acknowledged && (
                    <Button variant="outline" size="sm" onClick={() => handleAcknowledge(alert.id)} className="flex-shrink-0">
